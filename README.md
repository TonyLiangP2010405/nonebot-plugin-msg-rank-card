...
- 🥇 自动使用排行榜第一名的 QQ 头像作为背景
- 🕐 排行榜图片显示当前北京时间，发图前会播报日报/周报/月报类型
- 🖼️ 自动获取用户 QQ 头像
- 🏆 支持查看今日、本周和本月水群排行榜
- 🔔 支持 Superuser 开启排名变动提醒，名次变化后自动重发榜单
- ⏰ 支持按小时定时发送日榜，以及每天定时发送周榜和月榜
- 🧹 支持自动清理过期数据，默认保留35天
...
| msg_rank_auto_clean | 否 | true | 是否每天自动清理过期数据 |
| msg_rank_data_retention_days | 否 | 35 | 每日统计数据保留天数（最低31天） |
...
| 周水群榜 / 周榜 | 群员 | 否 | 群聊 | 查看本周一至今日的汇总排行榜 |
| 月水群榜 / 月榜 | 群员 | 否 | 群聊 | 查看本月1日至今日的汇总排行榜 |
| 排名变动提醒 开启\|关闭\|状态 | Superuser | 否 | 群聊 | 管理本群排名变动提醒 |
| 日榜定时 1-168\|关闭\|状态 | Superuser | 否 | 群聊 | 按指定小时间隔自动发送今日榜 |
| 周榜定时 HH:MM\|关闭\|状态 | Superuser | 否 | 群聊 | 每天指定时间自动发送本周榜 |
| 月榜定时 HH:MM\|关闭\|状态 | Superuser | 否 | 群聊 | 每天指定时间自动发送本月榜 |
| 清理水群数据 / clean_msgrank | 群主/管理员 | 否 | 群聊 | 清理过期数据 |

排名变动提醒默认关闭，开关按群保存。开启后，只有榜单成员或名次顺序发生变化时才会提醒；消息数增加但名次不变时不会重复发送。

定时任务默认关闭，按群保存，机器人重启后会自动恢复。`HH:MM` 使用 NoneBot APScheduler 配置的时区（默认 `Asia/Shanghai`）。
...
同时加载 `nonebot-plugin-mantou-affection` 后，日榜、周榜、月榜和定时榜单都会在每位成员旁显示
“馒头好感 + 当前关系称号 + 一句随好感阶段变化的馒头短评”。好感数据按群读取；
未加载好感度插件、好感度读取失败或该阶段没有文案时卡片保持原样，只是不显示短评。
短评取自好感度插件的 `msg_rank_card.rank` 场景（按 neutral/warm/close/flirty/intimate 五个阶段），
文案过长时会自动截断，不会盖住排名图案。

### 自定义资源

当第一名头像下载失败时，插件才会回退到 `resources` 文件夹中的背景图。你可以替换这些资源文件：
- `bg/` - 背景图片（随机选择）
- `frame/` - 排名边框装饰
